...
class WCMCA_Html
{
		...
		<div id="wcmca_form_popup_container_<?php echo $type; ?>">
			...
			<div class="woocommerce">
				<div  id="wcmca_address_form_<?php echo $type; ?>">
					...
					<!-- End error messages -->
					<input id="wcmca_address_id_<?php echo $type; ?>" name="wcmca_address_id" type="hidden" value="-1">
					<?php if(isset($user_id)): ?>
						<input type="hidden" name="wcmca_user_id" value="<?php echo $user_id;?>">
					<?php endif; ?>
						<?php 
						...
						foreach($address_fields as $field_name => $address_field)
						{
							...
							else if($field_name == 'billing_country' || $field_name == 'shipping_country')
							{
								...
								?> 
									...
									<img class="wcmca_preloader_image" src="<?php echo WCMCA_PLUGIN_PATH.'/img/loader.gif' ?>" />
									<div class="wcmca_divider"></div>
								<?php 
							}
							else
	...
	function render_address_select_menu($type = 'billing')
	{
		...
		<p>
			<img class="wcmca_loader_image" id="wcmca_loader_image_<?php echo $type; ?>" src="<?php echo WCMCA_PLUGIN_PATH.'/img/loader.gif' ?>" />
		</p>
		<?php 
	}
	public function render_add_product_address_for_guest_user($item_cart_id)
	{
		...
		<input type="hidden" name="wcmca_product_address_for_guest_user[<?php echo $item_cart_id; ?>]" id="product_address_for_guest_<?php echo $item_cart_id; ?>" value="same_as_billing">
		
		<img class="wcmca_product_address_loader" id="wcmca_product_address_loader_<?php echo $item_cart_id; ?>" src="<?php echo WCMCA_PLUGIN_PATH.'/img/horizontal-15.gif' ?>"/>
